...
Every screen ships both a dark and a light variant. The light mode is not a simple color inversion — it is a deliberate re-mapping that preserves the same hierarchy and visual weight on a white canvas.

### Color Mapping (Dark → Light)

| Role | Dark | Light |
|------|------|-------|
| Canvas / page background | `#0b0b0b` | `#ffffff` |
| Elevated surface (cards, inputs) | `#212121` | `#f0f0f0` |
| Subtle surface (badges, tags) | `#212121` | `#ededed` |
| Visible border | `#353535` | `#d8d8d8` |
| Subtle border / divider | `#212121` | `#e8e8e8` |
| Primary text | `#ffffff` | `#0b0b0b` |
| Secondary text | `#b9b9b9` | `#212121` |
| Tertiary text / metadata | `#797979` | `#797979` (unchanged) |
| CTA button (primary) | `#f36458` | `#f36458` (unchanged) |
| Hover / active state | `#0052ef` | `#0052ef` (unchanged) |
| Success | `#19d600` | `#19d600` (unchanged) |
| Error | `#dd0000` | `#dd0000` (unchanged) |
| Status icons (system bar) | `#ffffff` | `#0b0b0b` |

### Principles

- **CTA and interactive colors are invariant.** Coral-red (`#f36458`) and electric blue (`#0052ef`) appear identically in both modes. Only the neutral scale flips.
- **Tertiary gray (`#797979`) is invariant.** It sits midway on the achromatic scale and reads at sufficient contrast on both white and near-black canvases.
- **Elevated surfaces lighten instead of darkening.** In dark mode, cards rise from `#0b0b0b` toward `#212121`. In light mode, they descend from `#ffffff` toward `#ededed` — the same depth relationship, reversed.
- **Borders shift proportionally.** Dark mode hairlines (`#212121`) become light mode hairlines (`#e8e8e8`). The border is always one step away from the surface it sits on.
- **No warm or cool tinting.** Both modes use the same pure achromatic gray scale. Light mode is not cream or off-white — it is `#ffffff`.

### Light Mode Quick Reference

```
Background:      #ffffff
Surface:         #ededed (badges, tags) / #f0f0f0 (cards, inputs)
Border:          #d8d8d8 (visible) / #e8e8e8 (dividers)
Text Primary:    #0b0b0b
Text Secondary:  #212121
Text Tertiary:   #797979
CTA:             #f36458 (same as dark)
Interactive:     #0052ef (same as dark)
System icons:    #0b0b0b
```

### Implementation (Tailwind + shadcn)

Use semantic Tailwind tokens (`bg-background`, `text-foreground`, `border`) — never hardcode `bg-white` or `bg-[#0b0b0b]`. The theme switch is handled by toggling the `.dark` class on `<html>` via shadcn's `ThemeProvider`. CSS variables in `globals.css` define both sets of values.

```css
:root {
  --background: 0 0% 100%;        /* #ffffff */
  --foreground: 8 8% 5%;          /* #0b0b0b */
  --muted: 0 0% 93%;              /* #ededed */
  --muted-foreground: 0 0% 47%;   /* #797979 */
  --border: 0 0% 85%;             /* #d8d8d8 */
}

.dark {
  --background: 8 8% 5%;          /* #0b0b0b */
  --foreground: 0 0% 100%;        /* #ffffff */
  --muted: 0 0% 13%;              /* #212121 */
  --muted-foreground: 0 0% 47%;   /* #797979 */
  --border: 0 0% 21%;             /* #353535 */
}
```
